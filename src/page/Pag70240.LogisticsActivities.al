page 70240 "Logistics Activities"
{
    PageType = CardPart;
    ...
    layout
    {
        area(content)
        {
            cuegroup("Containers")
            {
                ...
                field("Delayed"; Rec."Delayed Containers")
                {
                    // ToolTip = 'Specifies the value of the Containers already Arrived field.', Comment = '%';
                    DrillDownPageId = "Containers list";
                    Caption = '⚠️ Delayed';
                    StyleExpr = DelayedStyleExp;
                }
    ...
    begin
        UpdateDelayedStyleExpr();
    end;

    local procedure UpdateDelayedStyleExpr()
    var
    begin
        DelayedStyleExp := 'Unfavorable';
    end;

    var
        DelayedStyleExp: text;
}
